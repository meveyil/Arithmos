...
export function Sidebar({
  ...
  const items: { id: AppSection; label: string; icon: typeof LayoutDashboard }[] =
    [
      ...
      { id: "archive", label: t.sidebar.archive, icon: Archive },
  ...
  return (
    <motion.aside
      ...
      className="thin-scrollbar flex w-[260px] min-w-[260px] shrink-0 flex-col gap-1 overflow-y-auto border-r border-zinc-200/80 bg-white/50 p-3 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-950/40"
    >
      ...
      {items.map(({ id, label, icon: Icon }, i) => {
        ...
        return (
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={() => onSelect(id)}
            className={`flex min-w-0 items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium transition-colors ${
              isOn
